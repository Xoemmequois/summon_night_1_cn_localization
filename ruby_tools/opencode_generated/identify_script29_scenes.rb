# ============================================================
# identify_script29_scenes — 完整提取 script29 所有 FID 场景对话
# ============================================================
# FID dispatch: switch c4[0x95] at 0x03F9 —
#   case 0 → 0x03C4: SET_FID 0x72 (初次事件)
#   case 1 → 0x03DD: SET_FID 0x73 (后续事件: 里普蕾生日)
# c4[0x95]=1 仅在 0x07FF 写入, 由 c4[0x21]==1 守卫 (0x0005@0x07F4 +
# 0x0021@0x07F9, 状态机 F9=7 handler 0x07DD 内)。
# c4[0x21] 是跨脚本全局旗标: script29 内唯一写入点位于孤儿子程序
# 0x084D (引擎回调, 脚本内零引用, 原始字节扫描确认), 故 VM 从
# 0x012E 起跑永远走不到 c4[0x21]=1 → 只提取 FID 0x72。
# 修复: 把 0x21 加入 external_vars — 在 0x0005 比较处 fork 两侧,
# regB=1 分支设 c4[0x95]=1 → F9=8 → 0x0274 → 0x03BC → FID 0x73。
# 注意不要把 0x95 设为 external: handle_0027 的 fork 不固化 case 值,
# 会导致两个 FID 各得相同的 243 条文本 (完全 crossover)。
# ============================================================
...
# ---------------------------------------------------------
def run_script29(commands, script_id)
  manager = ExecManager.new
  manager.index_to_pc = {}
  commands.each_with_index { |c, i| manager.index_to_pc[c.index] = i }

  # external_vars: default + 0x21 (全局事件旗标, 控制 FID 0x73 分支)
...
dialogs = run_script29(commands, SCRIPT_ID)
...
# ---------------------------------------------------------
puts "\n=== Combined Results ==="
output_results(all_dialogs, commands, "output_full/script29")

# Summary of FID coverage
puts
[0x72, 0x73].each do |fid|
  sub_id = fid + 0x29
