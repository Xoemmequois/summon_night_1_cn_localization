# ============================================================
# identify_script21_scenes — 完整提取 script21 所有 FID 场景对话
# ============================================================
# FID dispatch: switch var[0x95] at 0x0498 — 8 cases
#   case 2=0x0454→FID 0x52, case 3=0x0464→FID 0x53,
#   case 4=0x0474→FID 0x54, case 5=0x0484→FID 0x55
# Monkey-patch 跳过 c4[0x95]=2 写入 (0x03B5) + 遍历 c4[0x95]=2,3,4,5
# 0xC7 加入 external_vars 解决 0x2541 处的内部剧情 switch forking
# ============================================================
...
def run_with_c4_95(commands, script_id, c4_95_value)
  ...
  # external_vars: keep default + add 0xC7 (routing switch at 0x2541)
  #   + add 0xA5 (conditional guard at 0x09ED gating CALL 0x0B5F)
  # do NOT add 0x95 — let the FID dispatch switch route naturally
  manager.external_vars = VMState::EXTERNAL_VARS.dup + [0xC7, 0xA5]

  # Monkey-patch: skip the c4[0x95]=2 hardcode at 0x03B5 (F9=0xA handler)
...
# ---------------------------------------------------------
# Main: iterate c4[0x95]=2,3,4,5 for FIDs 0x52-0x55
# ---------------------------------------------------------

all_dialogs = {}

[2, 3, 4, 5].each do |val|
  puts "\n=== Run c4[0x95]=#{val} (targeting FID 0x#{sprintf('%02X', 0x52 + val - 2)}) ==="
...
# ---------------------------------------------------------
puts "\n=== Combined Results ==="
output_results(all_dialogs, commands, "output_full/script21")

# Summary of FID coverage
puts
(0x52..0x55).each do |fid|
  sub_id = fid + 0x29
